...
int p_kill(Cliente *cl, Cliente *bl, char *motivo, ...)
{
	...
	if (motivo)
	{
		...
		sendto_serv("%s %s %s :%s Usuario desconectado.", bl->trio, TOK_KILL, me.nombre, cl->trio);
...
int p_nuevonick(Cliente *al)
{
	...
	if (strchr(modos, 'r')) /* hay account */
		sendto_serv("%s %s %s 1 %lu %s %s +%s %s %s %s :%s", me.trio, TOK_NICK, al->nombre, time(0), al->ident, al->host, modos, al->nombre, encode_ip("127.0.0.1"), al->trio, al->info);
	else
		sendto_serv("%s %s %s 1 %lu %s %s +%s %s %s :%s", me.trio, TOK_NICK, al->nombre, time(0), al->ident, al->host, modos, encode_ip("127.0.0.1"), al->trio, al->info);
	return 0;
}
...
IRCFUNC(m_nick)
{
	if (parc > 3)
	{
		...
		if (parc > 8)
		{
			al = nuevo_cliente(parv[1], parv[4], parv[5], decode_ip(parv[parc - 3]), cl->nombre, parv[5], parc > 9 ? parv[6] : NULL, parv[parc - 1]);
