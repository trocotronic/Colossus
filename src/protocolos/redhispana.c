...
int p_svsmode(Cliente *cl, Cliente *bl, char *modos, ...)
{
	...
	va_end(vl);
	if (!strcmp(buf, "+r"))
		EnviaAServidor("%s %s %s", me.trio, TOK_IDENTIFY, cl->nombre);
	return 0;
}
...
int p_nuevonick(Cliente *al)
{
	...
	modos = ModosAFlags(al->modos, PROT_UMODOS(RH), NULL);
...
int PROT_CARGA(RH)(Conf *config)
{
	...
	{
		Error("[%s] La configuracion de %s es errónea", config->archivo, PROT_INFO(RH).nombre);
		return ++errores;
	}
...
void PROT_INICIA(RH)()
{
	...
	EnviaAServidor("PROTOCTL REQ :TOKEN");
		...
		switch(*mods)
		{
			...
			case 'k':
				...
				if (modo == ADD)
				{
					...
					cn->modos |= FlagAModo('k', PROT_CMODOS(RH));
				}
				else
				{
					if (cn->clave)
			...
			case 'l':
				if (modo == ADD)
				{
					...
					cn->modos |= FlagAModo('l', PROT_CMODOS(RH));
					param++;
				}
				else
				{
			...
			default:
				if (modo == ADD)
					cn->modos |= FlagAModo(*mods, PROT_CMODOS(RH));
				else
					cn->modos &= ~FlagAModo(*mods, PROT_CMODOS(RH));
		}
		mods++;
