...
<?
			require("_mysql.php");
			$query = mysql_query("SELECT dia, users FROM " . $db_pref . "stats WHERE DATE_SUB(CURDATE(),INTERVAL 1 MONTH) <= dia");
			$mf = 0;
			$i = 0;
			$avg = 0;
			...
			echo '</row><row><string>Maximo usuarios</string>';
			...
			echo '</row><row><string>Usuarios actuales</string>';
			echo str_repeat("<number>". (@USERS_CUR@) ."</number>\n", $i);
?>
		</row>
	</chart_data>
	...
	<draw>
		<text color='000000' alpha='30' font='arial' rotation='0' bold='true' size='31' x='0' y='0' width='400' height='150' h_align='left' v_align='top'>estadisticas por usuarios</text>
		<text color='666666' alpha='15' font='arial' rotation='-90' bold='true' size='40' x='-10' y='328' width='300' height='150' h_align='center' v_align='top'>nº usuarios</text>
