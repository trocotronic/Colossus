...
<?
			require("_mysql.php");
			$query = mysql_query("SELECT dia, servers FROM " . $db_pref . "stats WHERE DATE_SUB(CURDATE(),INTERVAL 1 MONTH) <= dia");
			$mf = 0;
			$i = 0;
			$avg = 0;
			...
			echo '</row><row><string>Maximo servidores</string>';
			...
			echo '</row><row><string>Servidores actuales</string>';
			echo str_repeat("<number>". (@SERVS_CUR@) ."</number>\n", $i);
?>
		</row>
	</chart_data>
	...
	<draw>
		<text color='000000' alpha='30' font='arial' rotation='0' bold='true' size='30' x='0' y='0' width='400' height='150' h_align='left' v_align='top'>estadisticas por servidores</text>
		<text color='666666' alpha='15' font='arial' rotation='-90' bold='true' size='40' x='-10' y='328' width='300' height='150' h_align='center' v_align='top'>nº servidores</text>
